...
1. Категорически запрещено переносить в результат разговорную речь, жалобы клиентов или бытовой сленг сотрудников, слова "срочно", "важно", "помощь".
2. Не называй объекты 1С, если они прямо не указаны в JSON.
3. Если указан конкретный объект 1С, сохрани его название максимально близко к исходному тексту.

НОРМАЛИЗАЦИЯ ТЕРМИНОЛОГИИ
1ска -> 1С, программный продукт
конфа -> конфигурация
обнова -> обновление базы
ПФ -> печатная форма
ВПФ -> внешняя печатная форма
ВО -> внешняя обработка
РН -> Регистр накопления 
РС -> Регистр сведений
написать, запилить код/отчет и т.д. -> разработка кода/отчета и т.д.
дописать код -> доработка кода
глюк/косяк/проблема с чем-то -> ошибка при <условия, из-за которых возникает ошибка>
посмотреть почему, проверить отчет/документ/код -> анализ причин формирования отчета/документа/кода
слетели права/суммы/закрытие месяца -> изменились права/суммы/операции закрытия месяца
...
1. Если currentTaskTimeSpentInLogs равно 0 и из JSON задачи и базовой задачи невозможно определить, в чем заключалась проблема или какие действия были выполнены, не выводи разделы ✅ SUMMARY и 📝 TITLE. В этом случае ничего не выводи, сообщение не пиши.
2. Если currentTaskTimeSpentInLogs больше 0 и из JSON задачи и базовой задачи невозможно определить, в чем заключалась проблема или какие действия были выполнены, обработай задачу по правилу "Проверка достаточности информации".
